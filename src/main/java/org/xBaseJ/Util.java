...
public class Util {
	...
	private static InputStream getPropertiesFile() {
		...
		if (f1.exists())
			try {
				propFile = f1;
				LOGGER.debug("properties file loaded from " + f1.getAbsolutePath());
			...
			if (f2.exists())
				try {
					propFile = f2;
					LOGGER.debug("properties file loaded from " + f2.getAbsolutePath());
				...
				if (f3.exists()) {
					try {
						propFile = f3;
						LOGGER.debug("properties file loaded from " + f3.getAbsolutePath());
					...
					if (is == null) {
						if (servletContextPath == null) // try it again
							...
							try {
								...
								LOGGER.debug("properties file loaded from servlet context path " + servletContextPath + "/org.xBaseJ.properties");
								propFile = new File(servletContextPath + "/org.xBaseJ.properties");

							}
							catch (IOException ioe) {
								LOGGER.debug("Searched for org.xBaseJ.properties as " + servletContextPath + "/org.xBaseJ.properties");
							}
						}
					}
					if (is != null) {
